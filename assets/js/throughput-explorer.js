...
function controls(){const p=sim().params;
  $('controls').setAttribute('data-scenario',active?'shared':'single');
  $('controls').innerHTML=(active?['rc','r','rs']:['rc','rs']).map(key=>`<div class="field"><label for="rate-${key}"><span>${{rs:'Server link &middot; R<sub>s</sub>',rc:'Client link &middot; R<sub>c</sub>',r:'Shared link &middot; R'}[key]}</span><output id="value-${key}" for="rate-${key}">${p[key]} Mbps</output></label><input id="rate-${key}" type="range" min="10" max="200" step="10" value="${p[key]}" data-rate="${key}"></div>`).join('');
