...
function controls(){const p=sim().params;
  $('controls').innerHTML=['rs','rc',...(active?['r']:[])].map(key=>`<div class="field"><label for="rate-${key}">${{rs:'Server link · Rₛ',rc:'Client link · R꜀',r:'Shared link · R'}[key]}<output id="value-${key}" for="rate-${key}">${p[key]} Mbps</output></label><input id="rate-${key}" type="range" min="10" max="200" step="10" value="${p[key]}" data-rate="${key}"></div>`).join('')+(active?`<div class="field"><label for="connections">Connections · N</label><select id="connections">${[2,3,4].map(n=>`<option ${p.n===n?'selected':''}>${n}</option>`).join('')}</select></div>`:'');
  ...
  if(active)$('connections').addEventListener('change',e=>{pause();sim().configure({n:Number(e.target.value)});render();});
  $('shared-preset').hidden=!active;
  $('scenario-title').textContent=active?'Multiple paths, one shared link':'One path, two links';
  $('scenario-caption').textContent=active?'Each colored connection receives an equal share R/N of the central link.':'A queue can form at the router before the client link.';
}
...
function link(x1,y1,x2,y2,rate,color,limited=false,drawBase=true,phase=0){x1=850-x1;x2=850-x2;let result=drawBase?`<line class="link ${limited?'limited':''}" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"/>`: "";
  // Every marker represents 5 Mbits; its fixed geometric speed does not depend on rate.
  if(rate>0&&!motion.matches){const length=Math.hypot(x2-x1,y2-y1),speed=130,spacing=speed/(rate/5),offset=(sim().time*speed+phase*spacing)%spacing;for(let d=offset;d<length;d+=spacing){const a=d/length;result+=`<circle cx="${x1+(x2-x1)*a}" cy="${y1+(y2-y1)*a}" r="3.5" fill="${color}"/>`;}}
  return result;
}
function buffer(x,y,amount,capacity,color,label,loss){const drop=motion.matches?12:(sim().time*35)%20;return `<rect x="${850-x-42}" y="${y}" width="84" height="12" rx="3" fill="#e5ecf1"/><rect x="${850-x-42}" y="${y}" width="${Math.min(84,84*amount/capacity)}" height="12" rx="3" fill="${color}"/>${text(x,y+27,`Queued: ${Math.round(amount)}/${Math.floor(capacity)} packets`)}${loss?`<circle cx="${850-x-45}" cy="${y+12+drop}" r="3" fill="#b62745"/>`+text(x,y+43,'↓ Dropping data','style="fill:#b62745;font-size:10px"'):''}`;}
function routerQueue(x,y,amounts,capacity,loss){
  ...
  markup+=text(x,y+28,`Queued: ${Math.round(total)}/${Math.floor(capacity)} packets`);
  if(loss){const drop=motion.matches?12:(sim().time*35)%20;markup+=`<circle cx="${left-5}" cy="${y+12+drop}" r="3" fill="#b62745"/>`+text(x,y+44,'Dropping data','style="fill:#b62745;font-size:10px"');}
  return markup+'</g>';
}
function diagram(){const s=sim(),p=s.params,b=s.prediction().bottlenecks;let svg='<title>Network capacity, throughput, and queues</title>';
  if(!active){svg+=link(105,145,370,145,p.rs,colors[0],b.includes('Server link'))+link(430,145,745,145,s.lastOut[0],colors[0],b.includes('Client link'));
    svg+=node(75,145,'Server')+node(400,145,'Router',true)+node(775,145,'Client');svg+=text(230,115,`Rₛ = ${p.rs} Mbps`)+text(600,115,`R꜀ = ${p.rc} Mbps`);svg+=buffer(400,215,s.clients[0],20,colors[0],'Queue',s.lastLoss[0]>0);
  }else{
    ...
    for(let i=0;i<p.n;i++){
      ...
      svg+=link(80,y,224,middle,p.rs,c,b.includes('Server link'));
      svg+=link(284,middle,549,middle,s.lastCore[i],c,false,false,i/p.n);
      svg+=link(609,middle,780,y,s.lastOut[i],c,b.includes('Client link'));
    ...
    svg+=node(254,middle,'Router 1',true)+node(579,middle,'Router 2',true);
    svg+=routerQueue(254,queueTop,s.core,Math.floor(20/p.n)*p.n,s.core.some(q=>q>=Math.floor(20/p.n)-1e-7)&&p.rs>p.r/p.n);
    svg+=routerQueue(579,queueTop,s.clients,20*p.n,s.clients.some((q,i)=>q>=20-1e-7&&s.lastCore[i]>p.rc));
  }
  if(!active)$('network').setAttribute('viewBox','0 90 850 180');$('network').innerHTML=svg;
}
function render(){const s=sim(),p=s.params,pred=s.prediction(),m=s.metrics();diagram();
  ...
  const row=(label,values,color,cls='')=>`<tr class="${cls}"><td>${color?`<span class="swatch" style="background:${color}"></span>`:''}${label}</td>${['recent','average','queue','lost'].map(k=>`<td>${k==='queue'||k==='lost'?Math.round(values[k]).toLocaleString():fmt(values[k])}</td>`).join('')}</tr>`;
  $('metrics').innerHTML=m.map((v,i)=>row('Connection '+(i+1),v,colors[i])).join('')+(active?row('Total',m.reduce((a,v)=>{for(const k in a)a[k]+=v[k];return a;},{recent:0,average:0,queue:0,lost:0}),null,'total'):'');
  const dropping=s.lastLoss.some(v=>v>1e-8),queued=m.some(v=>v.queue>1e-6);
  $('status').textContent=!s.time?'Ready':!running?'Paused':dropping?'Buffer full · dropping':queued?'Data queued':'Flowing freely';
...
function frame(now){if(previous!==null&&running){accumulator+=Math.min((now-previous)/1000,.1);while(accumulator>=ThroughputSimulation.DT){sim().step();accumulator-=ThroughputSimulation.DT;}}previous=now;if(running&&now-lastRender>80){render();lastRender=now;}requestAnimationFrame(frame);}
