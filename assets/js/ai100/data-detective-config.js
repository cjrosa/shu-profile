...
function fishInspectionOutput(code){const lines=code.split(/\r?\n/).map(x=>x.trim()).filter(x=>x&&!x.startsWith('#')),shown=[];for(const line of lines){if(/\.head\s*\(/i.test(line))shown.push(fishHeadOutput);if(/\.info\s*\(/i.test(line))shown.push(fishInfoOutput);if(/\.describe\s*\(/i.test(line))shown.push(fishDescribeOutput)}return shown.join('\n\n')||'No inspection output was requested.'}
...
window.ML_LAB_CONFIG={id:'data_detective',title:'Data Detective',description:'Use Python to turn a table of fish measurements into evidence you can see and explain.',learn:'How rows, features, labels, distributions, and relationships turn observations into evidence.',do:'Use Python tools to load and inspect Fish.csv with Pandas, then visualize distributions, species comparisons, and relationships with Seaborn.',instructorCue:'Pause and explain what the visual shows in your own words before moving on.',chartLabel:'Scatterplot of fish length and weight',metrics:{Rows:'159',Columns:'6',Species:'7',Missing:'0'},points:[[8,6,2,'Smelt'],[11,9,2,'Smelt'],[18,15,0,'Roach'],[22,26,0,'Roach'],[26,40,1,'Perch'],[30,58,1,'Perch'],[35,74,1,'Perch'],[39,91,4,'Pike'],[31,68,3,'Bream'],[34,82,3,'Bream'],[20,24,5,'Parkki'],[28,51,6,'Whitefish'],[43,105,4,'Pike'],[24,33,0,'Roach']],
scenes:[
 {kind:'dataset-intro',short:'Fish incoming!',title:'Watch the fish join the database',copy:'Every fish we observe swims into the database and becomes one saved row. Follow them in!',visualTitle:'The great fish data migration',visualText:'A fish is spotted. Splash! The database makes room and saves the observation.',species:['Roach','Perch','Smelt','Parkki','Pike','Bream','Whitefish'],colors:['#39c5f3','#ff6b6b','#ffb627','#a77bf3','#ff5ca8','#62d26f','#ff8a3d']},
 {short:'Meet the data',title:'A row is one observed fish',copy:'The dataset records 159 fish. Each row is an observation; each column records one property.',visualTitle:'Fish.csv · data preview',visualText:'Read this like a spreadsheet or database table. Column headers identify fields; each numbered row describes one fish.',table:{total:159,columns:[{name:'ID',type:'integer'},{name:'Species',type:'text'},{name:'Weight',type:'number'},{name:'Length',type:'number'},{name:'Height',type:'number'},{name:'Width',type:'number'}],rows:[[1,'Roach',27,19,6.4752,3.3516],[2,'Perch',5.9,7.5,2.112,1.408],[3,'Smelt',6.7,9.3,1.7388,1.0476],[4,'Smelt',7,10.1,1.7284,1.1484],[5,'Smelt',7.5,10,1.972,1.16],[6,'Smelt',8.7,10.8,1.9782,1.2852]]},definition:{term:'Observation',text:'One case or example represented by a row.'},cue:'The ID identifies the row, but it is not a measurement that describes the fish.'},
 {kind:'features-target',short:'Features & target',title:'Choose features based on the question',copy:'Suppose our question is: “Can a fish’s measurements help us estimate its weight?” Length, height, and width are input features (X). The known weight is the target (y).',definition:{term:'Feature',text:'An input measurement used to describe an example and help estimate the target.'},cue:'Read the row from left to right: Length, Height, and Width are features; Weight is the target. Species is context, not the answer to this question.'},
 {kind:'histogram',short:'Distributions',title:'A histogram groups values into ranges',copy:'Each bar counts fish whose weight falls inside that range. Read the bars together to see what is common, how far values spread, and whether the shape has a long tail.',definition:{term:'Distribution',text:'How observed values are spread across possible values.'},cue:'The bars touch because Weight is continuous. Bar height is a count—not an individual fish or a step in time.'},
 ...
 {kind:'purpose',short:'Why examine data?',title:'Use data examination to ask better questions',copy:'Before making a claim or building a model, use Python and Pandas to understand what the observations contain. Use Seaborn to turn those observations into charts, then use the Notebook to make the investigation repeatable.',visualTitle:'From raw observations to useful questions',visualText:'Each step answers a different question about the data. Pandas helps you inspect and summarize the table; Seaborn helps you visualize distributions, comparisons, and relationships before deciding what the evidence can support.',nodes:[{icon:'1',label:'Load with Pandas'},{icon:'2',label:'Inspect the table'},{icon:'3',label:'Summarize patterns'},{icon:'4',label:'Visualize with Seaborn'},{icon:'5',label:'Make careful claims'}],definition:{term:'Purpose of data examination',text:'Understand the structure, patterns, relationships, missing values, and limitations in observations before drawing conclusions.'},cue:'What question does each operation help answer: import pandas, read_csv, head(), info(), describe(), and a Seaborn visualization?'}
],
cells:[
 {objective:"Display a welcome message to practice running a Python statement.",takeaways:["print() displays text or values in the output.","Quotation marks tell Python to treat a message as text."],prompt:'Print a short welcome message.',requires:['print\\s*\\('],hint:'Use print("Data Detective") with straight quotes.',error:'NameError: name \'print\' is not used',coach:'Call Python’s print function with parentheses.',output:(state,code)=>(code.match(/print\s*\(\s*(['\"])(.*?)\1\s*\)/i)?.[2]||''),success:'Python evaluated your first statement.'},
 {objective:"Load Fish.csv into a pandas table for inspection.",takeaways:["pandas organizes data into rows and columns.","Each row represents one fish; each column records a property."],prompt:'Import pandas as pd, then load "Fish.csv" with pd.read_csv().',requires:['import pandas as pd','pd\\.read_csv\\s*\\(','fish\\s*='],hint:'Use two lines: import pandas as pd, then fish = pd.read_csv("Fish.csv").',error:'NameError: the fish table has not been created',coach:'Import pandas with the pd alias and assign read_csv() to a variable.',output:'Loaded Fish.csv: 159 rows × 6 columns',success:'The embedded course dataset is now available as a table.'},
 {objective:"Inspect sample rows, column types, and numeric summaries of the fish data.",takeaways:["head() previews rows; info() reports column types and non-missing counts.","describe() summarizes numeric values, including their center and spread.","A numeric ID identifies a record; its average is not a fish measurement."],prompt:'Inspect the first rows with head(), then request info() and describe().',requires:['\\.head\\s*\\(','\\.info\\s*\\(','\\.describe\\s*\\('],requireLabels:['head()','info()','describe()'],hint:'Call fish.head(), fish.info(), and fish.describe() on separate lines.',error:'AttributeError: inspection steps are incomplete',coach:'Use all three table-inspection methods requested above.',output:(state,code)=>fishInspectionOutput(code),success:'You inspected examples, structure, and numeric summaries.',metrics:{Rows:'159',Numeric:'5',Categories:'7',Missing:'0'}},
 {objective:"Visualize how fish weights are distributed with a histogram.",takeaways:["Each bar counts fish within a weight range.","The shape shows common weights, variation, and unusually large values."],prompt:'Import seaborn as sb and create a histogram of Weight.',requires:['import seaborn as (sb|sns)','(sb|sns)\\.histplot\\s*\\(','weight'],hint:'Use import seaborn as sb and sb.histplot(data=fish, x="Weight").',error:'NameError: histogram command not found',coach:'Import Seaborn and call histplot with the fish table and Weight column.',output:'<Weight histogram rendered in the visual panel>',visual:true,success:'The distribution view is ready.'},
 {objective:"Compare average fish weight across species with a bar plot.",takeaways:["Bar heights summarize the mean weight for each species.","The black lines show 95% confidence intervals for the estimated means, not the full range of fish weights."],prompt:'Create a bar plot of mean Weight for each Species.',requires:['(sb|sns)\\.barplot\\s*\\(','x\\s*=\\s*[\'\"]species','y\\s*=\\s*[\'\"]weight'],requireLabels:['barplot()','x="Species"','y="Weight"'],hint:'Use sb.barplot(data=fish, x="Species", y="Weight").',error:'ValueError: bar plot needs Species on x and Weight on y',coach:'Call Seaborn barplot with the fish table, Species as x, and Weight as y.',output:'<Mean Weight by Species bar plot rendered>',visual:true,success:'The bars compare estimated mean weight by species; the black lines show 95% confidence intervals.'},
 {objective:"Explore the relationship between fish length and weight across species.",takeaways:["Each point represents a fish; color identifies its species.","An upward pattern suggests that longer fish tend to weigh more.","An association does not establish that one measurement causes another."],prompt:'Create a scatterplot of Length and Weight, colored by Species.',requires:['(sb|sns)\\.scatterplot\\s*\\(','length','weight','hue\\s*=\\s*[\'\"]species'],hint:'Use sb.scatterplot(data=fish, x="Length", y="Weight", hue="Species").',error:'ValueError: scatterplot needs Length, Weight, and Species',coach:'Include x, y, and hue so the graph encodes all three requested fields.',output:'<Length × Weight scatterplot rendered>',success:'The chart now shows both association and species groups.',visual:true}
],
explore:{tasks:[
 {id:'distribution',title:'Inspect the distribution',objective:'Describe how fish weights are distributed.',directions:'Try changing the number of bins, identify the overall shape, and explain what the bars show.',kind:'histogram',controls:[{id:'bins',label:'Histogram bins',type:'range',min:6,max:18,value:10}],question:'Which description best matches the distribution?',options:['Roughly symmetric','Right-skewed','Uniform'],feedback:['Look again at where most fish are concentrated and which direction the sparse values extend.','Correct. Most fish are lighter, with a long tail toward heavier weights.','A uniform distribution would have bars of roughly equal height.'],completion:'Select Right-skewed and explain your answer.',evidence:'Most fish fall in the lighter-weight ranges, while a small number extend far into heavier ranges.'},
 {id:'relationship',title:'Investigate a relationship',objective:'Describe the relationship between fish length and weight.',directions:'Read the Length × Weight scatterplot, identify its direction and variation, then explain your answer.',kind:'scatterplot',question:'What relationship does the chart support?',options:['Longer fish tend to weigh more in this dataset.','Every fish gains exactly the same weight per centimeter.','Length and weight show no relationship.'],feedback:['Correct. Most dots rise from left to right, showing a positive association.','The vertical spread shows that the relationship is not exact.','The dots have a clear upward pattern from left to right.'],completion:'Select the positive association and explain your answer.',evidence:'Most dots rise from left to right, but fish of similar lengths can still have different weights.'},
