...
function fixture(config,storage=new Map()){
 ...
 const document={body:{},addEventListener(){},querySelector(s){const match=s.match(/^\[data-cell="(\d+)"\]$/);if(match)return cells[+match[1]];if(s.startsWith('[data-sidebar-task'))return null;if(s==='[data-mode="explore"]')return modes[2];if(!globals.has(s))globals.set(s,element());return globals.get(s)},querySelectorAll(s){if(s==='.ml-cell')return cells;if(s==='.ml-code')return cells.map(c=>c.querySelector(s));if(s==='.ml-mode')return modes;return []}};
...
test('ordered execution, Explore gating, invalidation and retained code',()=>{
 const f=fixture(simple()),{lab,cells}=f;cells.forEach(c=>c.querySelector('.ml-code').value='print("ok")');
...
test('paste budget, replacement, blocked/empty insertion, paired events, reload and reset isolation',async()=>{
 const f=fixture(simple()),{lab,cells}=f,e=cells[0].querySelector('.ml-code');lab.bindCodeInsertion(e,0);
 ...
 const restored=fixture(simple(),f.storage);restored.lab.restore();assert.equal(restored.lab.state.pastesUsed,3);
 ...
 const fresh=fixture(simple(),f.storage);fresh.lab.restore();assert.equal(fresh.lab.state.pastesUsed,0);
});
...
for(const name of ['data-detective','fish-predictor','hidden-patterns','loan-auditor'])test(name+' instructor snippets pass existing validation in order',()=>{
 ...
 const f=fixture(context.window.ML_LAB_CONFIG);f.lab.state.pastesUsed=3;f.lab.fillAllCode();assert.equal(f.lab.state.pastesUsed,3);assert.equal(f.lab.state.completed.size,0);f.cells.forEach((cell,i)=>assert.equal(f.lab.evaluate(cell,i),true));assert.equal(f.lab.notebookComplete(),true);
